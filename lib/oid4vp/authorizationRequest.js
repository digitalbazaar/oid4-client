...
export async function get({
  ...
  try {
    ...
    await validate({authorizationRequest, expectedClientId});
...
export async function validate({authorizationRequest, expectedClientId}) {
  ...
  // ensure `client_id` matches expected client ID (or a prefixed client ID for
  // backwards compatibility)
  if(expectedClientId !== undefined &&
    !_clientIdMatches({client_id, client_id_scheme, expectedClientId})) {
    throw createNamedError({
      message:
        `"client_id" in fetched request ("${client_id}") does not match ` +
        `authorization request URL parameter ("${expectedClientId}").`,
      name: 'DataError'
    });
  }
  ...
  // Note: This implementation requires `response_mode` to be one of:
  // `direct_post`
  // `direct_post.jwt`
  // `dc_api`
  // `dc_api.jwt`
  if(!(response_mode === 'direct_post' ||
    ...
    response_mode === 'dc_api' || response_mode === 'dc_api.jwt')) {
    throw createNamedError({
      message:
        `Response mode "${response_mode}" not supported; ` +
  ...
  // `direct_post.jwt` and `dc_api.jwt` response modes require encryption;
  // ensure the client meta data has the necessary parameters
  if(response_mode === 'direct_post.jwt' || response_mode === 'dc_api.jwt') {
    const {
...
function _clientIdMatches({client_id, client_id_scheme, expectedClientId}) {
  if(client_id === expectedClientId) {
    return true;
  }
  if(client_id_scheme === undefined) {
    return false;
  }
  if(expectedClientId.startsWith(client_id_scheme)) {
    return false;
  }
  const prefixedExpectedClientId = `${client_id_scheme}:${expectedClientId}`;
  return client_id === prefixedExpectedClientId;
}
