...
export async function create({
  ...
  encryptionOptions = _normalizeEncryptionOptions({encryptionOptions});
  ...
  if(authorizationRequest.response_mode === 'direct_post.jwt' ||
    ...
    // FIXME: `presentationSubmission` is not used in OID4VP 1.0+
    ...
    // Annex C (HPKE encrypted payload)
    body.Response = _encrypt({
      vpToken, presentationSubmission, authorizationRequest, encryptionOptions
    });
  } else {
...
async function _encrypt({
  ...
  if(encryptionOptions?.mdl?.handover?.type === 'dcapi') {
    ...
    const {
      enc, ct: cipherText
    } = await hpkeEncrypt({pt, authorizationRequest, encryptionOptions});
  ...
  const payload = {
    vp_token: vpToken,
    // FIXME: `presentation_submission` not used in Annex D, only B
    presentation_submission: presentationSubmission
  };
  return jwtEncrypt({payload, authorizationRequest, encryptionOptions});
}
...
function _normalizeEncryptionOptions({encryptionOptions}) {
