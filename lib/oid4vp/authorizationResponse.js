...
async function _encrypt({
  ...
  // determine if encrypting to a JWT or using HPKE; only mDL Annex C uses
  // HPKE at this time; handover type 'dcapi' === Annex C
  const encryptFn = encryptionOptions?.mdl?.handover?.type === 'dcapi' ?
    hpkeEncrypt : jwtEncrypt;
  return encryptFn({
    vpToken, presentationSubmission, authorizationRequest, encryptionOptions
  });
}
