...
export class OID4Client {
  ...
  async requestCredential({
    type, did, didProofSigner, agent, format = 'ldp_vc'
  } = {}) {
    const requests = [{type, format}];
  ...
  // FIXME: create a client from a credential offer
  ...
  static async fromPreAuthorizedCode({
    ...
    try {
      ...
      */
      ...
      return new OID4Client({accessToken, agent, issuerConfig});
    } catch(cause) {
...
function _assertRequest(request) {
  ...
  const {type, format} = request;
  if(typeof type !== 'string') {
    throw new TypeError('Credential request "type" must be a string.');
  }
  if(format !== undefined && typeof format !== 'string') {
    throw new TypeError('Credential request "format" must be a string.');
  }
}
