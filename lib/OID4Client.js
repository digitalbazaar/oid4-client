...
export class OID4Client {
  ...
  async requestCredential({
    credentialDefinition, getDidOptions, nonce, agent, format = 'ldp_vc',
    // deprecated; these should be returned via `getDidOptions()` instead
    did, didProofSigner
  } = {}) {
    // normalize to `getDidOptions`
    if(did && didProofSigner && !getDidOptions) {
      getDidOptions = () => ({did, didProofSigner});
    }

    // passing `credentialDefinition` and using `requests` is for
  ...
  async requestCredentials({
    requests, agent, nonce, format = 'ldp_vc',
    getDidOptions, alwaysUseBatchEndpoint = false,
    // deprecated; these should be returned via `getDidOptions()` instead
    did, didProofSigner
  } = {}) {
    // normalize to `getDidOptions`
    ...
    // if `nonce` is given, then DID options must also be
    if(nonce !== undefined && !getDidOptions) {
      throw createNamedError({
        message:
          'If "nonce" is given then "getDidOptions" or ' +
          '"did" and "didProofSigner" are required.',
        name: 'DataError'
      });
    }
        ...
        const results = await Promise.all(configuredRequests.map(
          async ({configuration, request}) => {
            ...
            return _requestCredential({
              ...
              url, json, nonce, getDidOptions, agent
            });
          }));
...
async function _addDIDProof({
  issuerConfig, credentialConfiguration, json, nonce, getDidOptions
}) {
  const {did, didProofSigner} = await getDidOptions({
    issuerConfig, credentialConfiguration, json
  });
  ...
  */
  ...
  for(let retries = 0; retries <= 1; ++retries) {
    try {
      ...
      // if DID options not provided, throw error
      if(!getDidOptions) {
        throw createNamedError({
