...
export async function send({
  ...
  try {
    ...
    const error = new Error(
      `Could not send OID4VP authorization response: ${cause.message}.`,
      {cause});
...
async function _encrypt({
  ...
  const {senderPrivateJwk} = encryptionOptions;
  if(encryptionOptions?.senderPrivateJwk) {
    // params: format, keyData, algorithm, extractable, keyUsages
    await webcrypto.subtle.importKey('jwk', senderPrivateJwk, {
      name: 'ECDH',
      namedCurve: 'P-256'
    }, true, ['deriveBits']);
  }
