...
function _primitiveValueToFilter(value) {
  ...
  if(VALUE_TYPES.has(filter.type)) {
    // We do not create a const for the empty string as that would cause an
    // equality check against the empty string. The query language in which we
    // translate from uses an empty string to signify an existence of a
    // property, therefore we have to do special processing to ensure the
    // semantics is correct when translated to the new one.
