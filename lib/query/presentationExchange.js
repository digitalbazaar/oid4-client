...
export function vprGroupsToPresentationDefinition({
  ...
  for(const queries of groups) {
      ...
      for(const credentialQuery of all) {
        ...
        if(acceptedEnvelopes?.includes('application/jwt')) {
          ...
          if(jwtVcJsonAlgs.length === 0) {
            jwtVcJsonAlgs.push(...SUPPORTED_JWT_ALGS);
          }
        }
    ...
    for(const didAuthentication of didAuthentications) {
      ...
      if(acceptedEnvelopes?.includes('application/jwt')) {
        if(jwtVpJsonAlgs.length === 0) {
          jwtVpJsonAlgs.push(...SUPPORTED_JWT_ALGS);
        }
      }
      if(acceptedCryptosuites) {
        const cryptosuites = acceptedCryptosuites
  ...
  const shouldAddFormat = jwtVcJsonAlgs.length > 0 ||
    ldpVcProofTypes.size > 0 || jwtVpJsonAlgs.length > 0 ||
    ldpVpProofTypes.size > 0;
