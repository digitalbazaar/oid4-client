...
export function dcqlQueryToVprGroups({dcqlQuery} = {}) {
  const {credentials} = dcqlQuery;
  let {credentialSets} = dcqlQuery;
  if(!credentialSets) {
    credentialSets = [{
  ...
  for(const credentialSet of credentialSets) {
    for(const option of credentialSet.options) {
      ...
      const queries = option.map(id => {
        ...
        return _toQueryByExampleQuery({dcqlCredentialQuery});
      });
...
export function vprGroupsToDcqlQuery({groupMap, options = {}} = {}) {
  ...
  for(const queries of groups) {
    ...
    credentialSets.options.push(option);
  }
