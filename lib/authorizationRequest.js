...
export async function get({url, agent} = {}) {
  try {
    ...
    _validate({authorizationRequest, expectedClientId});

    // resolve and validate any additional parameters in the request
    authorizationRequest = await _resolveParams({
...
async function _validate({authorizationRequest, expectedClientId}) {
...
async function _resolveParams({authorizationRequest, agent}) {
  const {
    client_metadata_uri,
    presentation_definition_uri,
    ...resolved
