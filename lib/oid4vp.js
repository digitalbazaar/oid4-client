...
export async function getAuthorizationRequest({url, agent} = {}) {
  try {
    ...
    if(requestUrl) {
      ...
      } = await _fetchAuthorizationRequest({requestUrl, agent}));
    }

    // ensure authorization request is valid
    _validateAuthorizationRequest({authorizationRequest, expectedClientId});

    // resolve and validate any additional parameters in the request
    authorizationRequest = await _resolveAuthorizationRequestParams({
...
async function _fetchAuthorizationRequest({requestUrl, agent}) {
...
async function _validateAuthorizationRequest({
  authorizationRequest, expectedClientId
}) {
...
async function _resolveAuthorizationRequestParams({
  authorizationRequest, agent
}) {
  const {
    client_metadata_uri,
  ...
  // FIXME: validate `authorizationRequest.client_metadata`
