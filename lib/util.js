...
export function toJsonPointerMap({obj, withShallowSets = false} = {}) {
  ...
  // FIXME: consider deep set map as well

  const shallowSetMap = new Map();
  for(const [pointer, value] of map) {
    const path = jsonpointer.parse(pointer);
    if(_isArrayElementPath(path)) {
      const key = `/${path.slice(0, -1).join('/')}`;
      const set = shallowSetMap.get(key);
      set ? set.add(value) : shallowSetMap.set(key, new Set([value]));
    } else {
      shallowSetMap.set(pointer, value);
    }
  }
  return shallowSetMap;
}

function _isArrayElementPath(path) {
  return !isNaN(parseInt(path.at(-1), 10));
}
