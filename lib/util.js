...
export function toJsonPointerMap({obj} = {}) {
  ...
  return _nextPointers({cursor: obj, map: new Map()});
}
...
function _nextPointers({cursor, map, tokens = [], pointer = '/'}) {
  if(Array.isArray(cursor)) {
    const set = new Set();
    map.set(pointer, set);
    for(const element of cursor) {
      // reset map, tokens, and pointer for array elements
      set.add(_nextPointers({cursor: element}));
    }
    return set;
  }
  if(_isObject(cursor)) {
    ...
    for(const [token, value] of entries) {
      ...
      _nextPointers({cursor: value, map, tokens, pointer});
      tokens.pop();
    }
    return map;
  }
